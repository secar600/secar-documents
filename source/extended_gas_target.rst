...
MFs are Mass Flow meters (also called mass flow controllers), which are devices to control the flow of the process gas. The MF have three states: :code:'CLOSED', :code:'OPEN', and :code:'SETPOINT' with the latter enabling the user to set a gas flow rate that will then be maintained by the MF. Each MF has a maximum flow the user can choose in :code;'SETPOINT' mode. This may be the maximum flow the specific MF has been designed to maintain (there are different types), or it may be set to a lower value for safety reasons. Note that setting the MF to the maximum flow is not the same as setting it to OPEN, even in cases where the maximum flow is the design maximum. 

One can right click on any MF on the CS-Studio control page, select "Open Mass Flow Controller Details" to open a popup window, from which the flow rate can be set, maximum allowed set point for that MF is also displayed (different MFs have different maximum flow rates), and the live flow readback is also shown. Note that currently this window does not indicate whether the MF is set to :code:'OPEN' or :code:'CLOSED' and displays a set point regardless. 

To determine and to change the state of the MF one needs to right click on the display field of the flow readback next to the MF symbol, select Probe, and manually remove the last part of the channel name after the : (usually R_FD) and replace it with :code:'MODE_CSET_MFC'. One than then view the value, and change it by typing :code:'CLOSED', :code:'OPEN', or :code:'SETPOINT'.

.. warning::

  Especially for :code:`MF2`, which regulates the flow of the supplied gas into the system,  setting a MF to :code:'OPEN' is not the same as setting it to its maximum allowed flow. The maximum flow protections only work when in :code:'SETPOINT' mode. If set to :code:`OPEN` the flow rate will not be known precisely. It may flood the system with gas so please avoid setting it to :code:`Set to OPEN`. 
